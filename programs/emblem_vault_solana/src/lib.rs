...
pub mod emblem_vault_solana {
    ...
    pub fn initialize_vault(ctx: Context<InitializeVault>, name: String, external_token_id: String) -> Result<()> {
        msg!("Initializing vault: {} with external_token_id: {}", name, external_token_id);
...
pub struct ClaimNFT<'info> {
    ...
    /// The `metadata` account is used in the metadata creation process, but no direct checks are required because the Metaplex metadata program enforces the necessary constraints.
    #[account(mut)] 
